...
// ========= Page (root) =========
export default function Kuchli100Page(): JSX.Element {
  ...
  const [days, setDays] = useState<Record<string, Record<string, boolean>>>(() =>
  ...
  const progress = useMemo(() => {
    const completed = Object.keys(days).filter((k) =>
      CHECKLIST.every((c) => days[k]?.[c.key])
    ).length;
  ...
  function toggleCheck(dayIndex: number, key: string) {
    setDays((prev) => {
      ...
      try {
        if (!USE_DEMO && profile?.userId) api.saveDay(profile.userId, dayIndex, next[dayIndex]);
      } catch {}
      return next;
    });
  }
  ...
  return (
    <div className="min-h-screen w-full bg-neutral-50 text-neutral-900">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                href="#join"
                className="px-5 py-3 rounded-2xl bg-emerald-600 text-white font-semibold hover:opacity-95 shadow"
              >
                Marafonga qo'shilish
              </a>
              <a
                href="#how"
                className="px-5 py-3 rounded-2xl border border-neutral-200 bg-white font-semibold hover:bg-neutral-100"
              >
                Qanday ishlaydi?
              </a>
            </div>
      ...
      <section
        id="how"
        className="px-5 md:px-10 py-12 md:py-16 bg-white border-t border-neutral-100"
      >
          ...
          {!profile ? (
            <form onSubmit={handleJoin} className="mt-6 grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-1 flex items-end">
                <button
                  disabled={loading}
                  className="w-full md:w-auto px-5 py-3 rounded-2xl bg-emerald-600 text-white font-semibold hover:opacity-95"
                >
                  {loading ? "Yuklanmoqda..." : "Boshlash"}
                </button>
              </div>
              <p className="md:col-span-2 text-xs text-neutral-500">
                Boshlash orqali shartlarga rozilik bildirasiz. Maxfiylik siyosati va qoidalar bilan tanishing.
              </p>
            </form>
          ) : (
            <div className="mt-4">
              <div className="flex flex-wrap items-center gap-3">
                <div className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-sm">
                  @{profile.username}
                </div>
                <button onClick={resetAll} className="text-sm underline">
                  Profilni tozalash (demo)
                </button>
              </div>
              <p className="mt-2 text-neutral-600 text-sm">
                Boshlash sanasi: <b>{startDate}</b>. Quyida har kungi belgilash paneli.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-4 grid md:grid-cols-2 gap-6">
            <div className="rounded-2xl border p-5">
              ...
              <a href="https://t.me/+" className="inline-block mt-3 px-4 py-2 rounded-xl bg-emerald-600 text-white">
                Kanalga qo'shilish
              </a>
            </div>
            <div className="rounded-2xl border p-5">
              ...
              <a href="https://instagram.com/_kuchlilar" className="inline-block mt-3 px-4 py-2 rounded-xl bg-neutral-900 text-white">
                Profilga o'tish
              </a>
            </div>
          </div>
...
function ShareButtons({ title = "Kuchli 100 Kun – Marafon", url }: { title?: string; url?: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <button onClick={share} className="px-4 py-2 rounded-xl border">
        Ulashish
      </button>
      <button onClick={copy} className="px-4 py-2 rounded-xl border">
        Havolani nusxalash
      </button>
    </div>
  );
...
function PublicLinkButton({ username }: { username?: string }) {
  const makeUrl = () => {
    const base =
      (typeof window !== "undefined" && window.location.origin) || "https://kuchlilar.com";
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={`text-xs px-2 py-1 rounded-full ${
            complete ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-600"
          }`}
        >
...
function LeaderboardSection() {
  ...
  useEffect(() => {
    ...
    api.leaderboard().then((res) => {
      if (m && res?.ok) setItems(res.items || []);
    });
    return () => {
      m = false;
    };
              ...
              {items.length ? (
                ...
                <tr className="border-t">
                  <td className="p-3" colSpan={4}>
                    Demo rejim. Backend ulanganida to‘ladi.
                  </td>
                </tr>
              )}
            </tbody>
